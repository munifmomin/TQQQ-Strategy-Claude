...
from tqqq_bvt.config import TICKERS, DATA_CACHE_PATH, START_DATE
...
def fetch_data(tickers: list[str] = TICKERS,
    ...
    if os.path.exists(DATA_CACHE_PATH) and not force_refresh:
        ...
        # check all requested tickers present
...
def align_to_spy(prices: pd.DataFrame) -> pd.DataFrame:
    """Align all series to SPY trading calendar; warn on gaps."""
